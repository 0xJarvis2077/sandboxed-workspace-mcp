...
The execution snapshot remains mounted only at `/workspace`. Each execution also receives a separate writable `/artifacts` mount and discovers it through `WORKSPACEGUARD_ARTIFACT_DIR=/artifacts`; the server never scans the workspace to guess which files are outputs. Only bounded top-level regular files are admissible from `/artifacts`; symlinks, directories, special files, control characters, and Unicode format controls fail closed.

Artifacts have two layers of truth. After an execution is terminal, the server re-validates staging, streams admitted bytes while enforcing limits and computing SHA-256, and commits bounded manifest metadata (ID, execution ID, name, media type, size, SHA-256, and creation time) in the same persistence transaction as the terminal `ExecutionRecord`, `ExecutionResources`, and state-transition event. SQLite v4 persists this manifest, but never binary bytes, host paths, resource URIs, or owner tokens. `execution_artifacts` uses the durable manifest as truth: `manifest_complete` distinguishes a known manifest for new executions from unavailable legacy history, while per-artifact `content_available` and nullable `resource_uri` describe only whether content is still available in the current process.

Artifact bytes remain process-local, ephemeral, and bounded by TTL, retained execution count, and total stored bytes, with whole-execution eviction; a server restart also removes bytes without deleting the durable manifest. Direct MCP Resource delivery has an additional 16 MiB upper bound so a large configured artifact cannot be loaded wholesale into server RAM. Final collector count, per-file, and total-byte admission limits are a **hard boundary for published artifacts**. The runtime `ArtifactGrowthMonitor` is sampled best-effort early enforcement, not a kernel filesystem quota; deleted-open files, rename/concurrent-mutation races, and same-user host interference remain residual risks.

Canonical terminal outcome preserves primary runtime truth. An existing `TIMED_OUT`, `CANCELLED`, `CRASHED`, or runtime `FAILED` outcome is not overwritten by a later artifact-finalization failure; artifact policy/admission/collection or cleanup failure can downgrade the overall execution only when the runtime outcome was originally `SUCCEEDED`.

For audit purposes, an execution consists of `ExecutionRecord` current state plus terminal `ExecutionResources`, `ExecutionEvent` lifecycle history, and a durable Artifact Manifest. Ephemeral Artifact Content is only the current byte-availability overlay on that manifest.
...
| `--execution-db` / `WORKSPACE_GUARD_MCP_EXECUTION_DB` | Optional ExecutionRecord + ExecutionEvent + Artifact Manifest SQLite database outside the workspace |
